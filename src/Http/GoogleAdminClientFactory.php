...
/**
 * Factory for creating Google Analytics Admin API clients.
 */
class GoogleAdminClientFactory implements LoggerAwareInterface
{
     ...
     */
    public function createAdminClient(array $config): AnalyticsAdminServiceClient
    {
        try {
            ...
            if (!empty($config['proxy'])) {
                ...
                $this->logger->info('Google Analytics Admin API using proxy', [
                    'proxy' => $config['proxy'],
                    'no_proxy' => $config['no_proxy'] ?? [],
                ]);
            }

            return new AnalyticsAdminServiceClient($clientConfig);

        } catch (CredentialsException $e) {
            $this->logger->error('Google Analytics Admin API credentials error', [
                'error' => $e->getMessage(),
            ]);
            throw $e;

        } catch (\InvalidArgumentException $e) {
            $this->logger->error('Invalid Google Analytics Admin API configuration', [
                'error' => $e->getMessage(),
            ]);
            throw $e;
